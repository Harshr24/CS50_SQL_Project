...
CREATE TABLE meteorites (name TEXT, id INTEGER PRIMARY KEY AUTOINCREMENT, class TEXT, mass REAL, discovery TEXT, year INTEGER, lat REAL, long REAL);
INSERT INTO meteorites (name, class, mass, discovery, year, lat, long) SELECT name, class, mass, discovery, year, lat, long FROM meteorites_temp WHERE nametype NOT LIKE '%Relict%' ORDER BY year ASC, name ASC;
